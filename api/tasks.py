...
def generate_6_digits_number():
    six_digit_code = ""
    code = [random.randint(1, 9) for n in range(6)]
    print(code)
    random.shuffle(code)
    for num in code:
        six_digit_code += str(num)
    return six_digit_code
   
# not the real implementation for login
# def send_otp_for_login(email):
#     OTP = generate_6_digits_number()
#     print(OTP)
#     subject = "OTP CODE"
#     html_content = render_to_string(
#         template_name="api/otp.html",
#         context={"subject": subject, "otp": OTP}
#     )
